...
from trade_hooks import (
    ...
    time_based_probe_exit, manage_trailing_and_partial, _notify_open_trade
)
...
def process_trend_symbol(*args, **kwargs):
    """
    process_trend_symbol(symbol, frames, indicators, votes, cfg, notifier, state, exec_engine, now_epoch)

    Nâng cấp:
      - Dùng ctx["mode"] = SCALPER|SWING để chỉnh SL/TP/trailing/time-based.
      - Dùng ctx["macro_regime"] = SIDEWAY_MACRO|TREND_ALIGN|CONFLICT:
          * CONFLICT: chỉ cho phép PROBE, không promote.
          * TREND_ALIGN: nới điều kiện promote.
    """
    ...
    mode        = (ctx.get("mode") or "SCALPER").upper()                  # NEW
    macro       = (ctx.get("macro_regime") or "SIDEWAY_MACRO").upper()    # NEW

    # Tham số theo mode
    if mode == "SWING":
        probe_sl_mult = float(cfg.get("engine", {}).get("early_probe", {}).get("sl_atr_mult", 1.4))
        probe_tp_mult = float(cfg.get("engine", {}).get("early_probe", {}).get("tp_atr_mult", 1.2)) * 1.2
        trailing_mult = float(cfg.get("engine", {}).get("manage", {}).get("trailing_atr_mult", 1.2))
        trailing_mult = min(trailing_mult, 1.0)  # SWING chặt: ~1.0 ATR
        tbe_minutes   = int(cfg.get("engine", {}).get("time_based_probe_exit", {}).get("max_minutes", 20)) * 3
    else:  # SCALPER
        ...
        tbe_minutes   = int(cfg.get("engine", {}).get("time_based_probe_exit", {}).get("max_minutes", 20)) // 2 or 10
    ...
    try:
        ...
        if bool(early_cfg.get("enabled", True)):
            if should_open_early_probe(side_m15=side_m15, anti_chase_ok=anti_ok, snapshot_ok=snapshot_ok):
                # macro CONFLICT → vẫn cho probe nhưng cẩn trọng
                ...
                size_quote = float(early_cfg.get("probe_size_quote", cfg.get("probe_size_quote", 0)))
                _open_trade_safe(exec_engine, symbol, side_m15, price, sl, tp, size_quote,
                                 setup="TREND", notifier=notifier, reason=f"early_probe(mode={mode},macro={macro})")
    except Exception:
        pass
    ...
    try:
        ...
        # macro CONFLICT → KHÔNG promote
        allow_promote = (macro != "CONFLICT")
        # macro TREND_ALIGN → nới điều kiện promote
        ...
        if allow_promote and promote_ready:
            ...
            if active:
                sl_mult = float(prom_cfg.get("sl_atr_mult", cfg.get("atr_sl_mult", 1.6)))
                tp_mult = float(prom_cfg.get("tp_atr_mult", 1.8))
                # SWING → TP tham vọng hơn một chút
                if mode == "SWING": tp_mult *= 1.1
                if side_m15=="LONG":
                    new_sl = price - sl_mult * atr; new_tp = price + tp_mult * atr
                else:
                    new_sl = price + sl_mult * atr; new_tp = price - tp_mult * atr
                add_quote = float(prom_cfg.get("promote_add_size_quote", cfg.get("promote_add_size_quote", 0)))
                try:
                ...
                for t in list(opens):
                    ...
                    if tp_level > 0 and hasattr(sim, "partial_close"):
                        try:
                            ...
                            # SWING giữ lệnh lâu hơn → chốt nhẹ hơn
